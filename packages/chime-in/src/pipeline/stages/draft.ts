...
export async function draftReply(args: {
  ...
  while (attempts < MAX_ATTEMPTS) {
    ...
    const res = await args.llm.complete({
      tier: "draft",
      system,
      prompt: buildDraftPrompt({ ...args, shortenFrom, fixDashesFrom, maxChars: target }),
      schema: DraftResultSchema,
      label: `draft:${args.post.tweet_id}${shortenFrom ? ":shorten" : fixDashesFrom ? ":dashes" : ""}`,
      maxTokens: 500,
    });
    ...
    if (hasDashTell(reply) && attempts < MAX_ATTEMPTS) {
      fixDashesFrom = reply;
      continue;
    }
    break;
  ...
  // A dash that survived the rewrite budget is surfaced, not silently shipped.
